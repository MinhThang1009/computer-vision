...
def render(
    ...
    motion_bias=[torch.tensor([0, 0, 0])],
    rotation_bias=[torch.tensor([0, 0])],
    scales_bias=[1, 1],
):
    """
    Render the scene.
    ...
    for pc in gaussians:
        if screenspace_points is None:
            ...
            screenspace_points = torch.cat(
                [screenspace_points, screenspace_points1], dim=0
            )
    try:
    ...
    for index, pc in enumerate(gaussians):
        means3D_final1, scales_final1, rotations_final1, opacity_final1, shs_final1 = (
            get_state_at_time(pc, viewpoint_camera)
        )
        ...
        if index == 0:
            ...
            means3D_final1 = rotate_point_cloud(
                means3D_final1, motion_bias_t, rotation_bias_t, scales_bias[index - 1]
            )
            # breakpoint()
...
parser.add_argument(
    "--configs1", type=str, default="arguments/dynerf_9/flame_salmon_1.py"
)
parser.add_argument(
    "--configs2", type=str, default="arguments/dnerf_tv_2/hellwarrior.py"
)
...
if args.configs1:
    from mmengine.config import Config  # env-compat: mmcv→mmengine
    from utils.params_utils import merge_hparams

    config = Config.fromfile(args.configs1)
...
if args2.configs2:
    ...
    config = Config.fromfile(args2.configs2)
...
if args3.configs3:
    ...
    config = Config.fromfile(args3.configs3)
    args3 = merge_hparams(args3, config)
    args3.model_path = args3.modelpath3
safe_state(args.quiet)
gaussians1, scene1, background = init_gaussians(
    model.extract(args1),
    hyperparam.extract(args1),
    args1.iteration,
    pipeline.extract(args1),
    args1.skip_train,
    args1.skip_test,
    args1.skip_video,
)
gaussians2, scene2, background = init_gaussians(
    model.extract(args2),
    hyperparam.extract(args2),
    args2.iteration,
    pipeline.extract(args2),
    args2.skip_train,
    args2.skip_test,
    args2.skip_video,
)
gaussians3, scene3, background = init_gaussians(
    model.extract(args3),
    hyperparam.extract(args3),
    args3.iteration,
    pipeline.extract(args3),
    args3.skip_train,
    args3.skip_test,
    args3.skip_video,
)
...
for index, viewpoint in tqdm(enumerate(scene1.getVideoCameras())):
    result = render(
        ...
        gaussians,
        bg_color=background,
        motion_bias=[torch.tensor([4, 4, 12]), torch.tensor([-2, 4, 12])],
        rotation_bias=[
    ...
    torchvision.utils.save_image(
        result["render"], os.path.join(render_path, f"output_image{index}.png")
    )

imageio.mimwrite(
    os.path.join(render_path, "video_rgb.mp4"), render_images, fps=30, codec="libx265"
)
