...
\begin{figure}[H]
    ...
    \caption[Pipeline 4D-GS]{Pipeline 4D-GS (nhóm vẽ lại theo bài báo~\cite{wu20244dgs}). Tọa độ
...
\centering
\small
\caption[Kết quả reproduce trên D-NeRF]{Kết quả reproduce trên 8 cảnh D-NeRF (test set) so với số liệu bài báo.}
\label{tab:reproduce}
...
(tương đương 5--7 phút trên T4); đa số cảnh chỉ tinh chỉnh thêm dưới 3 dB sau đó, riêng vài cảnh chuyển động mạnh (standup, mutant) tăng tới $\sim$5 dB. Biểu đồ
...
\begin{figure}[H]
    ...
    \caption[Montage render/ground-truth D-NeRF]{D-NeRF: render (hàng trên) so với ground-truth (hàng dưới) trên cả 8 cảnh test.}
...
\centering
\small
\caption[Kết quả reproduce trên HyperNeRF]{Reproduce trên 4 cảnh vrig HyperNeRF (test set, 14.000 vòng) so với số
...
\begin{figure}[H]
    ...
    \caption[Montage render/ground-truth HyperNeRF]{HyperNeRF: render (hàng trên) so với ground-truth (hàng dưới) trên 4 cảnh vrig.}
...
\centering
\small
\caption[So sánh các phương pháp trên D-NeRF]{So sánh các phương pháp trên bộ D-NeRF ($800\times800$), số liệu trích từ
...
nó mã hóa: video thật đơn camera, vài trăm khung hình, độ phân giải thấp. Chỉ điều kiện (ii) bị thiếu: tác giả 4D-GS cung cấp point
...
\begin{figure}[H]
    ...
    \caption[Montage render/ground-truth NeRF-DS]{NeRF-DS: render (hàng trên) so với ground-truth (hàng dưới) trên cả 7 cảnh test.}
...
\centering
\small
\caption[4D-GS trên ba dataset (đối sánh chéo)]{4D-GS (mô hình nhóm huấn luyện) trên ba dataset, trung bình mỗi bộ. Cột
...
\begin{figure}[H]
    ...
    \caption[Biểu đồ PSNR/SSIM/LPIPS ba dataset]{So sánh 4D-GS trên ba dataset (PSNR/SSIM/LPIPS). \textbf{PSNR} giảm dần
...
Hồ sơ năng lực của 4D-GS (huấn luyện khoảng 10 phút đến hơn một giờ mỗi cảnh, mô hình 18 MB, render
...
và tổng hợp \textbf{đối sánh chéo ba dataset}: chất lượng 4D-GS giảm dần theo trục
