...
\usepackage{hyperref}
\definecolor{linkblue}{rgb}{0.12,0.39,0.78}
\hypersetup{
  colorlinks=true,
  linkcolor=linkblue,   % link nội bộ: mục lục, \ref, \label
  citecolor=linkblue,   % \cite
  urlcolor=blue         % \href URL ngoài
}
...
34,06 dB), xác nhận tính tái lập. Đối sánh chéo ba dataset cho thấy chất lượng 4D-GS
giảm dần theo chiều tổng hợp $\to$ thật $\to$ phản chiếu; riêng trên NeRF-DS, phương
...
tái lập (reproduce), tổng quát hóa dataset, vật liệu phản chiếu, spherical harmonics, overfitting đơn camera (monocular).
...
biểu diễn của cảnh và render ra ảnh tại \textit{một góc nhìn mới}. Với
...
Hướng thứ ba xuất phát từ 3D-GS: Dynamic 3DGS~\cite{dynamic3dgs} lưu trạng thái của các Gaussian tại
...
định nghĩa bởi tâm $\mathcal{X}$ và ma trận hiệp phương sai $\Sigma$:
\begin{equation}
    G(x) = \exp\Big(-\tfrac{1}{2}(x-\mathcal{X})^{T}\Sigma^{-1}(x-\mathcal{X})\Big).
    \label{eq:gaussian}
\end{equation}
...
2.000 lên hàng chục đến hàng trăm nghìn.
...
Gaussian chuẩn đến trạng thái của thời điểm $t$}, rồi để nguyên quy trình
...
Toàn bộ quy trình được nhóm vẽ lại trong Hình~\ref{fig:pipeline}.
...
\begin{figure}[H]
    ...
    \caption[Quy trình 4D-GS]{Quy trình 4D-GS (nhóm vẽ lại theo bài báo~\cite{wu20244dgs}). Tọa độ
...
dạng cũng tương tự. Bài báo gọi đây là điểm khác biệt then chốt so với việc học
...
\begin{figure}[H]
    ...
    \caption[Tối ưu hai giai đoạn coarse-to-fine]{Quá trình tối ưu coarse-to-fine: khởi tạo bằng Gaussian tĩnh giúp mô
...
Nhóm đã chạy lại (reproduce) toàn bộ quy trình của bài báo bằng mã nguồn chính
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{HyperNeRF}~\cite{park2021hypernerf} (vrig): 4 cảnh \textit{thật} thu bằng 1--2 camera chuyển động đơn giản (huấn luyện theo chế độ monocular), $960\times540$ (bài báo cũng đánh giá).
...
\section{Kết quả tái lập và đối sánh với bài báo}
...
\centering
\small
\caption[Kết quả tái lập trên D-NeRF]{Kết quả tái lập trên 8 cảnh D-NeRF (test set) so với số liệu bài báo.}
\label{tab:reproduce}
...
\textbf{tái lập được}. Tốc độ render đạt $\sim$30--44 FPS trên T4, thấp hơn con số
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption[Render so với ground-truth (mutant, lego)]{So sánh ảnh render của mô hình nhóm huấn luyện với ground-truth trên
...
(tương đương 5--7 phút trên T4); đa số cảnh chỉ tinh chỉnh thêm dưới 3 dB sau đó, riêng vài cảnh chuyển động mạnh (standup, mutant) còn tăng thêm $\sim$3,5--4,5 dB. Biểu đồ
...
\begin{figure}[H]
    ...
    \caption[Hội tụ PSNR test trên D-NeRF]{Tiến trình PSNR test của 8 cảnh D-NeRF theo vòng lặp giai đoạn fine
...
\begin{figure}[H]
    ...
    \caption[Ảnh ghép render/ground-truth D-NeRF]{D-NeRF: render (hàng trên) so với ground-truth (hàng dưới) trên cả 8 cảnh test.}
    \label{fig:montage_dnerf}
\end{figure}

\section{Tái lập trên HyperNeRF (dữ liệu thật)}
\label{sec:hypernerf}
...
\centering
\small
\caption[Kết quả tái lập trên HyperNeRF]{Tái lập trên 4 cảnh vrig HyperNeRF (test set, 14.000 vòng) so với số
...
Là dữ liệu thật (thu bằng 1--2 camera) nhiễu hơn D-NeRF, HyperNeRF thường khó tái lập sát tuyệt
đối hơn cảnh tổng hợp; điểm cần khẳng định là kết quả nằm trong vùng sai số hợp lý
quanh số công bố, qua đó xác nhận quy trình chạy đúng trên \textit{cả} dữ liệu tổng
...
\begin{figure}[H]
    ...
    \caption[Ảnh ghép render/ground-truth HyperNeRF]{HyperNeRF: render (hàng trên) so với ground-truth (hàng dưới) trên 4 cảnh vrig.}
...
\begin{figure}[H]
    ...
    \caption[Hội tụ PSNR test trên HyperNeRF]{Tiến trình PSNR test của 4 cảnh HyperNeRF theo vòng lặp giai đoạn fine
...
\section{Đối sánh với các phương pháp khác}

Bảng~\ref{tab:sota} trích số liệu trung bình trên D-NeRF từ bài báo, bổ sung cột
kết quả tái lập của nhóm.

\begin{table}[H]
\centering
\small
\caption[Đối sánh các phương pháp trên D-NeRF]{Đối sánh các phương pháp trên bộ D-NeRF ($800\times800$), số liệu trích từ
...
4D-GS (nhóm, Colab T4)       & 34,06 & 0,98 & 0,03 & $\sim$13,5 phút & 38,3 & $\sim$24,8 \\
\hline
\end{tabular}%
}
\end{table}
...
\textbf{NeRF-DS}~\cite{yan2023nerfds}: bộ dữ liệu thật, dựng đơn-view (rig train/test như HyperNeRF), quay các
...
khá trên cảnh hình học chiếm ưu thế nhưng suy giảm rõ khi vật phản chiếu lấn át khung hình.
...
tái dùng cấu hình mặc định của HyperNeRF vì NeRF-DS tương đồng về mọi đặc tính mà
nó mã hóa: video thật dựng đơn-view, vài trăm khung hình, độ phân giải thấp. Chỉ điều kiện (ii) bị thiếu: tác giả 4D-GS cung cấp point
cloud COLMAP dựng sẵn cho HyperNeRF nhưng dĩ nhiên không có cho NeRF-DS.

\subsection*{Các điều chỉnh kỹ thuật}

Thay vì chạy lại toàn bộ quy trình COLMAP dense (nhiều giờ trên Colab), nhóm tận
dụng point cloud 3D có sẵn của NeRF-DS, vốn sinh ra từ chính bước đăng ký camera
...
\centering
\small
\caption[Kết quả 4D-GS trên NeRF-DS]{4D-GS trên 7 cảnh NeRF-DS (test set, $480\times270$, 14.000 vòng): số nhóm
...
\hline
\textbf{Trung bình} & 22,76 & 0,8220 & 0,2188 & 22,79 & 81,6 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption[Render so với ground-truth (sieve, plate)]{Kết quả trên NeRF-DS. Cảnh \textit{sieve} (25,72 dB) giữ được hình
...
\begin{figure}[H]
    ...
    \caption[Ảnh ghép render/ground-truth NeRF-DS]{NeRF-DS: render (hàng trên) so với ground-truth (hàng dưới) trên cả 7 cảnh test.}
...
\begin{figure}[H]
    ...
    \caption[Hội tụ PSNR test trên NeRF-DS]{Tiến trình PSNR test của 7 cảnh NeRF-DS theo vòng lặp giai đoạn fine
    (eval tại 3.000/7.000/14.000). Số liệu từ log huấn luyện (tensorboard, tập test
    lấy mẫu), dùng đọc xu hướng; có thể lệch nhẹ so với số \texttt{metrics.py} ở bảng per-scene.}
...
quy trình tái lập đúng số công bố trên cả dữ liệu tổng hợp (D-NeRF) lẫn dữ liệu
...
\begin{figure}[H]
    ...
    \caption[Trục 1: đối sánh nhóm với số công bố]{Trục 1: đối sánh kết quả nhóm với số công bố trên từng dataset
    (D-NeRF: PSNR/SSIM/LPIPS-vgg; HyperNeRF: PSNR/MS-SSIM; NeRF-DS: PSNR/SSIM/LPIPS-vgg
...
Các mục trên đối sánh từng dataset với số công bố (Trục 1, trả lời ``tái lập có đúng không'').
Mục này tổng hợp theo \textbf{Trục 2} (đối sánh chéo): đặt kết quả 4D-GS do nhóm huấn
luyện (cùng cấu hình, riêng từng cảnh) trên ba dataset cạnh nhau để đọc \textbf{hành vi của phương pháp theo độ khó
...
\centering
\small
\caption[Trục 2: đối sánh chéo ba dataset]{Trục 2: đối sánh chéo 4D-GS (mô hình nhóm huấn luyện) trên ba dataset, trung bình mỗi bộ. Cột
...
NeRF-DS   & thật, phản chiếu & 22,76 & 0,8220 & 0,2188 & 81,6 & 45,4 & $\sim$59.500 \\
\hline
\end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption[Trục 2: đối sánh chéo ba dataset]{Trục 2: đối sánh chéo 4D-GS trên ba dataset, theo ba chỉ số PSNR, SSIM và LPIPS-vgg. \textbf{PSNR} giảm dần
    theo chiều tổng hợp (D-NeRF) $\to$ thật (HyperNeRF) $\to$ thật phản chiếu (NeRF-DS);
    riêng SSIM/LPIPS-vgg không đơn điệu do khác độ phân giải và cảnh (xem phần ``Đọc kết quả'').}
    \label{fig:cross}
\end{figure}
...
\textit{không} theo cùng chiều: NeRF-DS (SSIM 0,8220 cao hơn, LPIPS 0,2188 thấp hơn) thậm chí tốt hơn
HyperNeRF (0,6853 / 0,3374)\footnote{SSIM thường ở đây khác với MS-SSIM 0,841 của HyperNeRF ở Bảng~\ref{tab:hypernerf}; là hai chỉ số khác nhau.}, một phần vì cảnh \textit{broom2} kéo tụt trung bình của
HyperNeRF, một phần vì NeRF-DS độ phân giải thấp hơn ($480\times270$) nên dễ đạt
SSIM cao và LPIPS thấp (xem ``Lưu ý đọc bảng''). Do đó kết luận 4D-GS suy giảm trên vật phản chiếu dựa
chủ yếu vào PSNR và đối sánh per-scene với SpectroMotion
(mục~\ref{sec:nerfds}), không dựa SSIM/LPIPS cross-dataset.
Xu hướng này khớp luận điểm nhóm đặt ra từ phân tích phương pháp (mục~\ref{sec:deform});
FPS, dung lượng và số Gaussian thì giữ ổn định. Riêng NeRF-DS tụt rõ so với HyperNeRF dù
cả hai đều là dữ liệu thật góc nhìn hạn chế, cùng chế độ dựng đơn-view (monocular, rig train/test), khác biệt nổi bật là vật
...
\begin{figure}[H]
    ...
    \caption[FPS theo số Gaussian mỗi cảnh]{Quan hệ tốc độ render (FPS) và số Gaussian mỗi cảnh; mỗi điểm là một
...
Đóng góp giá trị nhất của 4D-GS là đưa render cảnh động thời gian thực thành kết quả kiểm chứng được. Sự khác biệt với các phương pháp
...
nằm ở quyết định kiến trúc phân tích tại chương~\ref{chap:method}: giữ nguyên quy trình splatting
...
18 MB (10 MB Gaussian cộng 8 MB trường biến dạng) theo bài báo; nhóm đo được
20--102 MB tùy số Gaussian mỗi cảnh. Dung lượng là hằng số theo
...
lại ra đúng số công bố là một bằng chứng cho thấy phương pháp được công bố trung thực. Cuối cùng,
...
Các hạn chế của 4D-GS có thể chia thành ba loại theo nguồn gốc. Loại thứ nhất
...
xuất hiện rồi biến mất), thể hiện qua chính trường hợp thất bại chiếc chổi trong phụ
lục bài báo. Cùng loại này là phát hiện riêng của nhóm ở mục~\ref{sec:nerfds}:
...
Loại thứ hai đến từ \textbf{chế độ dữ liệu}. Phương pháp phụ thuộc mạnh vào
...
liệu thật, nhóm đo được khoảng cách train-test tới 12 dB (cảnh \textit{sieve}) trên NeRF-DS, một mức
...
cũng chỉ nhỉnh hơn TiNeuVox-B khoảng 0,8 dB (25,13 so với 24,3 trong bảng HyperNeRF của bài báo~\cite{wu20244dgs}) trong khi huấn luyện lâu gấp đôi, tức lợi
...
Loại thứ ba mang tính \textbf{kỹ thuật triển khai} nhưng ảnh hưởng thực tế không
...
Hồ sơ năng lực của 4D-GS (huấn luyện khoảng 10 phút đến hơn một giờ mỗi cảnh, mô hình $\sim$20--102 MB, render
...
áp đặt (trường hợp thất bại chiếc chổi); pose nhiễu hoặc thiếu điểm nền làm gãy khâu
...
Nhóm cũng áp dụng phương pháp lên \textbf{NeRF-DS} (vật phản chiếu, ngoài bài báo): kết quả
khớp số 4D-GS độc lập của SpectroMotion (PSNR 22,76 so với 22,79), xác nhận quy trình áp dụng đúng.
Phần \textbf{đối sánh chéo ba dataset} cho thấy chất lượng 4D-GS giảm dần theo chiều
...
C.-D. Fan, C.-W. Chang, Y.-R. Liu, J.-Y. Lee, J.-L. Huang, Y.-C. Tseng, Y.-L. Liu.
